...
import { Declaracion } from "../Variables/Declaracion";

export class ForOf extends Instruccion {

  constructor(private id: string, private declaracion: Declaracion | null, private Contenedor: Expresion,
  ...
  public traducir(entorno: Entorno) {
    ...
    if (this.declaracion != null) {
      if (contenedor.getTipo() == Tipo.ARRAY) {
        this.declaracion.tipo = { tipo: Tipo.NUMBER, dim: 0}
      } else { 
        this.declaracion.tipo = { tipo: Tipo.STRING, dim: 0}
      }
    ...
    generador.setToStack(tElemento, t3);
